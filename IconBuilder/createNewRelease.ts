...
const createNewRelease = (libType: string) => {
    console.debug('++createNewRelease begain');

    del([
        path.resolve(__dirname, `./newRelease/${libType}/**/*`)
    ]).then( () => {
        // fs.mkdirSync(path.resolve(__dirname, `./newRelease/${libType}`));
        fs.mkdirSync(path.resolve(__dirname, `./newRelease/${libType}/react`));

        const esDir = path.resolve(__dirname, `./icons/${libType}/react/es`);
        const tgtEsDir = path.resolve(__dirname, `./newRelease/${libType}/react/es`);
        const stylesDir = path.resolve(__dirname, `./icons/${libType}/react/styles`);
        const tgtStylesDir = path.resolve(__dirname, `./newRelease/${libType}/react/styles`);
        const packageFile = path.resolve(__dirname, `./icons/${libType}/react/package.json`);
        const tgtPackageFile = path.resolve(__dirname, `./newRelease/${libType}/react/package.json`);
        const iconsMapFile = path.resolve(__dirname, `./icons/${libType}/react/iconsMap.json`);
        const tgtIconsMapFile = path.resolve(__dirname, `./newRelease/${libType}/react/iconsMap.json`);
        ...
        del([
            ...
            // path.resolve(__dirname, `icons/${libType}`)
